...
#     # Sort the files based on the epoch number extracted from the filename
#     pth_files_sorted = sorted(pth_files, key=lambda x: int(x.split('_')[-1].replace('.pth', '')))

#     # Return the files from index 30 to 50 (1-based index becomes 0-based)
#     return pth_files_sorted[30:50]
...
model = '/content/GraphBAN/case_study/best_model_epoch_45.pth'
...
new_data['predicted_value'] = df_test['pred']

new_data.to_csv(args.save_dir, index=False)
